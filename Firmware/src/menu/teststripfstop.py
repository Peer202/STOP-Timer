...
class TestStripFStopPage(MenuPage):
    def __init__(self,menu):
        self.menutitle = "Test Strip Mode FStop"
    ...
    def update_menu_screens(self):
        ...
        for i,param in enumerate(self.parameters):
            if(i == 1):
                param_string = param[0] + "1/" + str(param[1])
            else:
    ...
    def on_increment(self):
        ...
        if(self.selected_parameter_index== 1):
            #FStop Change doubles
            self.parameters[self.selected_parameter_index][1] = param[1] * 2
        else:
    ...
    def on_start(self):
        ...
        self.mainmenu.force_new_menu_layer(
            TestStripStartPageFstop(self.mainmenu,
            min=self.parameters[0] [1],
            dF=self.parameters[1] [1],
...
class TestStripStartPageFstop():
    def __init__(self,menu,min,dF,steps):
        ...
        self.enlarger_on_duration = []
        self.min = min
        self.vis_steps = []
        factor_increase = 1 + (1 / dF)
        for i in range(steps):
            if ( i == 0 ):
                self.vis_steps[0] = self.min
                self.enlarger_on_duration.append(self.min)
            else:
                self.vis_steps.append(round(min * factor_increase,1))
                self.enlarger_on_duration.append(self.vis_steps[i]- self.vis_steps[(i-1)])
    ...
    def on_start(self):
        ...
        running_page_object = EnlargerRunningPage(self.mainmenu,self.enlarger_on_duration[self.current_step_index],self.on_enlarger_done)
